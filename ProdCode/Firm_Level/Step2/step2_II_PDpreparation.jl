function  step2_II_PDpreparation(PathStruct, DataMonth, smeEcon, countrycode = 9)
    ...
    UpdatedFirmHistory = SharedArray{Float64, 2}((0, 6))
    @everywhere econs = $econs

    UpdatedFirmHistory = @sync @distributed (vcat) for i = 1:length(econs)
        # global UpdatedFirmHistory
        matread(PathStruct["CompanyInformationFolder"]*"FirmHistory_"*string(econs[i])*".mat")["firmHistory"][:,[1,2,3,8,10,11]]
    ...
    if !isfile(PathStruct["FullPeriodPD"]*"pdAllForwardwithfirminfo.jld")
        pdAllForward = @sync @distributed (hcat) for i = 1:length(econs)
            ...
            [econPDAll; econFirmInfo']#,
        end
        save(PathStruct["FullPeriodPD"]*"pdAllForwardwithfirminfo.jld", "pdAllForwardwithfirminfo", pdAllForward, compress = true)
    else
