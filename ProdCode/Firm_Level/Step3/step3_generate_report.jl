function step3_generate_report(path_to_input_file, DataMonth, smeEcon = [1 3 9 10], countrycode = 9)
    ...
    for i = 1:length(num)
        ...
        #= firmpreDTD with 18 cols, for current stage, left the last 5 cols unfilled.
            3:  NI/TA     4:  sales/TA         5:  TL/TA        6:  CASH/TA     7:  CASH/CL
            8:  CL/TL     9:  LB/TL            10: BE/TL        11: BE/CL       12: TA in million
            13: TA/TL     14: risk free rate   15: stock index return           16: forex rate
        ...
        idf, idmedian =
            ismember_CK(hcat(firmpreDTD[i][:, 2], countrycode*ones(size(firmpreDTD[i], 1))), DTDmedian[:, 1:2], "rows")
        firmpreDTD[i][BitArray(idf), 17:18] = DTDmedian[idmedian, 3:4]  ## DTDmedian  3. DTD   4. 1/sigma
        ...
        #= DTD regressors in firmpreDTD cols
            3:  NI/TA         6:  CASH/TA         7:  CASH/CL        8:  CL/TL     11: BE/CL
        ...
        #= firmspecific cols
            1: Company code   2: yyyy    3: mm     4: rfr   5:  index return   6: DTD
