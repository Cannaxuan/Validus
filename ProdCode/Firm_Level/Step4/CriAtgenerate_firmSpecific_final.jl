function CriAtgenerate_firmSpecific_final(PathStruct, DTDmedian, beta, SampleM, FirmIndex, lb, ub, countrycode)
    ...
    DTDInput = JldTools.cri_read_jld(PathStruct["Firm_DTD_Regression_FS"]*"DTDInput.jld")["DTDInput"]
    DTDInput = DTDInput[:, vcat(1:3, 19)]
    DTDInput = convert(DataFrame, DTDInput)
    names!(DTDInput,  [:CompNo, :monthDate, :industryID, :BE])
    ...
    fSpecific = JldTools.cri_read_jld(PathStruct["Firm_Specific"]*"firmSpecific_afterNormalize_beforeAverDiff_"*
                string(countrycode)*".jld")["firmSpecific_afterNormalize_beforeAverDiff"]

    dateVctr = nanMean(fSpecific[:,2,:], 2)*100 + nanMean(fSpecific[:,3,:], 2)
    ...
    # end
    return firmspecificAll, firmlistAll, dateVctr
end
