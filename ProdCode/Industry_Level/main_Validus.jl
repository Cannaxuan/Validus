...
function main_Validus(dataEndDate,PathStruct,smeEcon=[1 3 9 10])
     ...
     if loadSMEInfo && isfile(PathStruct["SMEinfoFolder"]*"smeInfo.mat")
         smeInfo = matread(PathStruct["SMEinfoFolder"]*"smeInfo.mat")["smeInfo"]
         smeEcon = matread(PathStruct["SMEinfoFolder"]*"smeInfo.mat")["smeEcon"]
     else
         ctyInfo, smeInfo = generate_SME_info(smeEcon, PathStruct["DATE_START_DATA"], dataEndDate,
                                              facs["dateVctr"], options, PathStruct)
